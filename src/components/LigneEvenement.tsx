...
export default function LigneEvenement(
  ...
  return (
    <tr style={{ opacity: pending ? .5 : 1 }}>
      ...
      <td><strong>{evenement.titre}</strong><br />
      ...
      <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
        <Link className="btn btn-y btn-sm" href={`/admin/evenements/${evenement.id}`}>Modifier</Link>{' '}
        <button
          className="btn btn-w btn-sm"
          onClick={() => {
            if (confirm(`Supprimer "${evenement.titre}" ? Cette action est définitive.`))
